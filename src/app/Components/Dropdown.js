...
function Dropdown({ data, platformSelected, handlePlatformMenu }) {
  return (
    <Menu as="div" className="relative inline-block text-left">
      ...
      >
        <Menu.Items className="absolute right-0 z-10 md:w-[12rem] w-[100%] text-[18px] mediumfont origin-top-right rounded-xl bg-white shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
          <div className="py-1">
            {data?.map((item, index) => (
              <Menu.Item
                key={index}
